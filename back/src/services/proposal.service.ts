    ...
    for (const p of otherProposals) {
      if (p.id !== acceptedProposalId) {
        ...
        this.socketHandler?.sendNotification(p.proposerId, 'proposal:rejected', {
          message: `Uma outra proposta para "${p.item.titulo}" foi aceita.`,
          itemId: p.itemId,
        });
        // A notificação persistente já é enviada ao atualizar o status
        // await this.notificationService.notifyProposalRejected(p.proposerId, p.item.titulo);
      }
    }
  }
