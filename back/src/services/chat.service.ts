...
export class ChatService {
  ...
  /**
   * Busca mensagens entre dois usuários, com filtro opcional por item
   */
  async getMessages(userId: number, otherUserId: number, itemId?: number) {
    ...
    // Filtra por item se informado
    if (itemId) {
      whereConditions[0].itemId = itemId;
      whereConditions[1].itemId = itemId;
    }
